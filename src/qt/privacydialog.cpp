                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zDREG ought to be enough for anybody." - Bill Gates, 2017
    ui->zDREGpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzDREGSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzDREG->setEnabled(false);
        ui->pushButtonMintzDREG->setToolTip(tr("zDREG is currently disabled due to maintenance."));

        ui->pushButtonSpendzDREG->setEnabled(false);
        ui->pushButtonSpendzDREG->setToolTip(tr("zDREG is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zDREGpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzDREG_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDREG is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zDREG...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zDREG in ") + 
...
void PrivacyDialog::on_pushButtonSpendzDREG_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDREG is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zDREG
        sendzDREG();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zDREG
    sendzDREG();
}

void PrivacyDialog::on_pushButtonZDREGControl_clicked()
{
    ZDREGControlDialog* zDREGControl = new ZDREGControlDialog(this);
    zDREGControl->setModel(walletModel);
    zDREGControl->exec();
}

void PrivacyDialog::setZDREGControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzDREGSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzDREG()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Dregulate Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zDREGpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zDREG is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " DREG </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zDREG</b>";
    ...
    // use mints from zDREG selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZDREGControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZDREGControlDialog::GetSelectedMints();
    }
    ...
    // Clear zDREG selector in case it was used
    ZDREGControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zDREG Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " DREG, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zDREG Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zDREG </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zDREG "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zDREG "));
    ui->labelzDREGAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzDREGSyncStatus->setVisible(fShow);
}
