...
#include "ui_zDREGcontroldialog.h"
...
std::list<std::string> ZDREGControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZDREGControlDialog::listMints;

ZDREGControlDialog::ZDREGControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDREGControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZDREGControlDialog::~ZDREGControlDialog()
{
    delete ui;
}

void ZDREGControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDREGControlDialog::updateList()
{
...
void ZDREGControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDREGControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZDREG_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDREGControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZDREGControlDialog::GetSelectedMints()
{
...
void ZDREGControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
